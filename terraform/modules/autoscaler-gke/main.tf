...
terraform {
  provider_meta "google" {
    module_name = "cloud-solutions/memorystore-cluster-autoscaler-deploy-gke-v3.0.0" // x-release-please-version
  }
}
...
resource "google_service_account" "otel_collector_service_account" {
  ...
  display_name = "Memorystore Cluster Autoscaler - SA for OpenTelemetry Collector in ${var.name}"
}
...
resource "google_service_account" "gke_cluster_service_account" {
  ...
  display_name = "Memorystore Cluster Autoscaler - cluster SA for ${var.name}"
}
...
resource "google_artifact_registry_repository" "autoscaler_artifact_repo" {
  location      = var.region
  repository_id = "memorystore-cluster-autoscaler"
  description   = "Image registry for Memorystore Cluster Autoscaler"
  format        = "DOCKER"
}
