...
describe('#linear', () => {
  describe('calculateSize', () => {
    ...
    it('scales OUT, but avoids invalid config of 4', async () => {
      const cluster = createClusterParameters({
        currentSize: 3,
        metrics: [
          {
      ...
      // It should scale to 4, but it's invalid, so it scales to 5.
      assert.equal(suggestedSize, 5);
    });

    it('scales IN, but avoids invalid config of 4', async () => {
      const cluster = createClusterParameters({
        currentSize: 5,
        metrics: [
          {
